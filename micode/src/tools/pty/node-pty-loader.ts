// src/tools/pty/node-pty-loader.ts
// Loads node-pty-android-arm64 with graceful degradation for Termux/Android ARM64.
...
/**
 * Dynamically load node-pty-android-arm64 (via npm alias) with graceful degradation.
 * Uses npm alias: `npm install node-pty@npm:node-pty-android-arm64`
 * This keeps `require('node-pty')` working in code while loading the Android ARM64 build.
 *
...
export async function loadNodePty(): Promise<NodePtyModule | null> {
  ...
  try {
    ...
    log.info(LOG_TAG, "node-pty loaded successfully");
    return ptyModule;
  } catch (error) {
    loadError = extractErrorMessage(error);
    const firstLine = loadError.split("\n")[0];
    log.warn(LOG_TAG, `node-pty unavailable: ${firstLine}`);
