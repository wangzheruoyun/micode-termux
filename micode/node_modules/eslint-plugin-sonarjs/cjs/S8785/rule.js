...
const async_analysis_js_1 = require("./async-analysis.js");
const call_to_declaration_js_1 = require("../helpers/call-to-declaration.js");
/**
...
const TEST_AND_SUITE_NAMES = new Set([...mocha_style_test_frameworks_js_1.TEST_FUNCTION_NAMES, ...mocha_style_test_frameworks_js_1.SUITE_FUNCTION_NAMES]);
const messages = {
    removeAsync: 'Remove the "async" keyword from this test suite callback; a test suite callback must be synchronous.',
    asyncSuiteCallback: 'Move this async setup into a lifecycle hook or a test callback; any test declared after this "await" is silently dropped.',
    removeAsyncQuickFix: 'Remove the "async" keyword',
    droppedTest: 'This test is declared after the await and is never registered.',
    asyncHelperCall: 'This async helper silently drops tests; remove its "async" keyword and move async setup into a lifecycle hook or a test callback.',
};
exports.rule = {
    meta: (0, generate_meta_js_1.generateMeta)(meta, { messages, hasSuggestions: true }),
    create(context) {
        if (!(0, module_js_1.importsOrDependsOnModule)(context, SUPPORTED_FRAMEWORKS, SUPPORTED_FRAMEWORKS)) {
            return {};
        }
...
function visitRaisingSuiteCallback(context, parserServices, raisingSuiteCallback) {
    ...
    if ((0, parser_services_js_1.isRequiredParserServices)(parserServices)) {
        reportUnawaitedAsyncHelperCalls(context, parserServices, callback, asyncSuiteOutcome?.kind === 'awaitedHelperReported' ? asyncSuiteOutcome.skipAwait : undefined);
    }
}
function analyzeAsyncSuiteCallback(context, parserServices, callback) {
    const topLevelAwait = (0, async_analysis_js_1.findFirstTopLevelAwait)(context, callback);
    if (!topLevelAwait) {
        return { kind: 'noTopLevelAwait' };
    }
    const awaitedHelperOutcome = tryReportAwaitedAsyncHelperCall(context, parserServices, callback, topLevelAwait);
...
function tryReportAwaitedAsyncHelperCall(context, parserServices, callback, topLevelAwait) {
    if (!(0, parser_services_js_1.isRequiredParserServices)(parserServices)) {
        return undefined;
    }
    const awaitedCall = (0, async_analysis_js_1.getAwaitedCall)(topLevelAwait);
    if (!awaitedCall) {
        return undefined;
    }
    const nextDescribeAwait = callback.body?.type === 'BlockStatement'
        ? findNextTopLevelAwaitAfter(context.sourceCode, callback.body, topLevelAwait)
        : undefined;
    const reported = reportAsyncHelperCall(context, parserServices, awaitedCall, callback, topLevelAwait, nextDescribeAwait);
    if (!reported) {
        return undefined;
    }
    return { kind: 'awaitedHelperReported', skipAwait: topLevelAwait };
}
function reportAsyncSuiteCallback(context, callback, topLevelAwait) {
    const secondaryLocations = (0, async_analysis_js_1.findTestsRegisteredAfter)(context.sourceCode, callback, topLevelAwait, TEST_AND_SUITE_NAMES).map(test => (0, location_js_1.toSecondaryLocation)(test, messages.droppedTest));
 ...
 */
function getRaisingSuiteCallback(context, node, parserServices) {
    const parts = (0, mocha_style_test_frameworks_js_1.getMochaCalleeParts)(node.callee);
    ...
    for (const argument of node.arguments) {
        ...
        const decl = (0, call_to_declaration_js_1.followReferenceToDeclaration)(argument, parserServices);
        if (!decl) {
            continue;
        }
        const esTreeDecl = parserServices.tsNodeToESTreeNodeMap.get(decl);
        if (!esTreeDecl || !(0, async_analysis_js_1.isFunctionLike)(esTreeDecl)) {
            continue;
        }
 ...
 */
function isSupportedFrameworkConstruct(context, identifier) {
    const variable = (0, ast_js_1.getVariableFromScope)(context.sourceCode.getScope(identifier), identifier.name);
    if (variable && variable.defs.length > 0) {
        const fqn = (0, module_js_1.getFullyQualifiedName)(context, identifier);
        if (fqn === null) {
            return false;
        }
...
function reportRemoveAsync(context, callback, withQuickFix) {
 ...
 */
function reportUnawaitedAsyncHelperCalls(context, services, callback, skipAwait) {
    ...
    const stack = body.type === 'BlockStatement'
        ? [...(0, ancestor_js_1.childrenOf)(body, sourceCode.visitorKeys)]
        : [body];
    while (stack.length > 0) {
        const current = stack.pop();
        if (!current || (0, ast_js_1.isFunctionNode)(current)) {
            continue;
        }
        let expr = null;
        ...
        const nextDescribeAwait = body.type === 'BlockStatement'
            ? findNextTopLevelAwaitAfter(sourceCode, body, expr)
            : undefined;
        reported =
            reportAsyncHelperExpression(context, services, expr, callback, skipAwait, nextDescribeAwait) || reported;
    }
    return reported;
}
function reportAsyncHelperExpression(context, services, expr, callback, skipAwait, nextDescribeAwait) {
    ...
    if (expr.type === 'AwaitExpression' &&
        ...
        return reportAsyncHelperCall(context, services, expr.argument, callback, expr, nextDescribeAwait);
    }
    return false;
}
function findNextTopLevelAwaitAfter(sourceCode, body, afterNode) {
...
function shouldSkipHelperResolution(context, call) {
    const calleeParts = (0, mocha_style_test_frameworks_js_1.getMochaCalleeParts)(call.callee);
    const isKnownFrameworkRegistration = calleeParts !== undefined &&
        test_frameworks_js_1.TEST_FRAMEWORK_STRUCTURE_FUNCTIONS.has(calleeParts.base.name) &&
        isSupportedFrameworkConstruct(context, calleeParts.base);
    return isKnownFrameworkRegistration || (0, assertion_detection_js_1.isAssertion)(context, call);
}
/**
 ...
 */
function reportAsyncHelperCall(context, services, callNode, describeCallback, awaitNode, nextDescribeAwait) {
    const decl = (0, call_to_declaration_js_1.followCallToDeclaration)(callNode, services);
    if (!decl) {
        return false;
    }
    // Only flag async functions.
    if (!(typescript_1.default.getCombinedModifierFlags(decl) & typescript_1.default.ModifierFlags.Async)) {
        return false;
    }
    ...
    const esTreeDecl = services.tsNodeToESTreeNodeMap.get(decl);
    if (!esTreeDecl || !(0, async_analysis_js_1.isFunctionLike)(esTreeDecl)) {
        return false;
    }
    // Find the first top-level await inside the helper.
    const helperAwait = (0, async_analysis_js_1.findFirstTopLevelAwait)(context, esTreeDecl);
    if (!helperAwait) {
        return false;
    }
    // Require at least one test dropped after the await inside the helper.
    const droppedInHelper = (0, async_analysis_js_1.findTestsRegisteredAfter)(context.sourceCode, esTreeDecl, helperAwait, TEST_AND_SUITE_NAMES);
    ...
    const droppedInDescribe = awaitNode
        ? (0, async_analysis_js_1.findTestsRegisteredAfter)(context.sourceCode, describeCallback, awaitNode, TEST_AND_SUITE_NAMES, nextDescribeAwait)
        : [];
    const secondaryLocations = [...droppedInHelper, ...droppedInDescribe].map(test => (0, location_js_1.toSecondaryLocation)(test, messages.droppedTest));
    (0, location_js_1.report)(context, {
