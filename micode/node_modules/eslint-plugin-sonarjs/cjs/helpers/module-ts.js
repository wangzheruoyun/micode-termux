...
exports.importsModuleTS = importsModuleTS;
/*
...
const FILE_IMPORTS_CACHE = new WeakMap();
...
function importsModuleTS(sourceFile, moduleNames) {
...
function getImportsOfSourceFile(sourceFile) {
...
function getImportedModuleName(statement) {
...
function getRequiredModuleNames(variableStatement) {
...
function getRequiredModuleName(initializer) {
    ...
    if (!typescript_1.default.isCallExpression(initializer) || !isRequireCall(initializer)) {
        return undefined;
    }
    const moduleName = initializer.arguments[0];
    return typescript_1.default.isStringLiteral(moduleName) ? moduleName.text : undefined;
}
