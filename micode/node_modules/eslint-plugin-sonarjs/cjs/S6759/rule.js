...
function isReadOnly(props, services) {
    ...
    return properties.every(property => isPropertyReadonlyIncludingBaseTypes(type, property, checker));
}
function isPropertyReadonlyIncludingBaseTypes(type, property, checker, seen = new Set()) {
    const name = property.getEscapedName();
    if ((0, ts_api_utils_1.isPropertyReadonlyInType)(type, name, checker)) {
        return true;
    }
    ...
    if (!(property.flags & typescript_1.default.SymbolFlags.Transient)) {
        return false;
    }
    if (!(type.flags & typescript_1.default.TypeFlags.Object) || seen.has(type)) {
        return false;
    }
    seen.add(type);
    const objectType = type;
    if (!(objectType.objectFlags & typescript_1.default.ObjectFlags.ClassOrInterface)) {
        return false;
    }
    const baseTypes = checker.getBaseTypes(objectType);
...
function getPropertyFromType(type, name) {
