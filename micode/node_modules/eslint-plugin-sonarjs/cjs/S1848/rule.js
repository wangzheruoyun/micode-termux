 ...
 * ```
 */
function isRegExpValidation(node, context) {
    ...
    return node.arguments.some(argument => (0, ast_js_1.isIdentifier)(argument) && containsReturnedIdentifier(returnArgument, argument));
}
function isBuiltInRegExpConstructor(node, context) {
    const { callee } = node;
    const scope = context.sourceCode.getScope(node);
    if ((0, ast_js_1.isIdentifier)(callee, 'RegExp')) {
        return !(0, ast_js_1.getVariableFromScope)(scope, callee.name)?.defs.length;
    }
    return (callee.type === 'MemberExpression' &&
        !callee.computed &&
        (0, ast_js_1.isIdentifier)(callee.object, 'globalThis') &&
        !(0, ast_js_1.getVariableFromScope)(scope, callee.object.name)?.defs.length &&
        (0, ast_js_1.isIdentifier)(callee.property, 'RegExp'));
}
function getNextSiblingStatement(node, context) {
    const ancestors = context.sourceCode.getAncestors(node);
...
function getStatementList(node) {
...
function containsReturnedIdentifier(node, identifier) {
    if ((0, ast_js_1.isIdentifier)(node, identifier.name)) {
        return true;
    }
    ...
    if (node.type === 'ConditionalExpression') {
        return (containsReturnedIdentifier(node.consequent, identifier) ||
            containsReturnedIdentifier(node.alternate, identifier));
    }
    const nodeType = node.type;
    if (nodeType === 'TSAsExpression' || nodeType === 'TSTypeAssertion') {
        return containsReturnedIdentifier(node.expression, identifier);
    }
    return false;
}
 ...
 */
function isGlobalFormException(context, callee, calleeText) {
    if (calleeText === 'Notification') {
        return true;
    }
    if (calleeText === 'window.ClipboardJS') {
        const variable = (0, ast_js_1.getVariableFromName)(context, 'window', callee);
    ...
    for (const rootName of ['paper', 'paperScope']) {
        ...
        const variable = (0, ast_js_1.getVariableFromName)(context, rootName, callee);
